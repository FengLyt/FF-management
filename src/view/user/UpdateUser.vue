  ...
  <div>
    <el-dialog title="用户管理" :visible.sync="dialogFormVisible" width="400px">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitForm('ruleForm') ,UserSave()">更新</el-button>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      this.$http.post("/admin/user/updateUser", this.form).then(resp => {
    ...
    getData(id) {
      this.$http.get("admin/user/getUserById?id=" + id).then(resp => {
        this.form.account = resp.data.data.account;
        this.form.role = resp.data.data.role;
        this.form.dept.id = resp.data.data.dept.id;
